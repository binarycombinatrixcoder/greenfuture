...
export async function getProductPage() {
  const getProductQuery = groq`*[_type == "product"][]
  {
    'categories': categories[]->title,
    'slug':slug.current,
    'desc':desc,
    publishedAt,
    title,
    'mainImages': mainImages[].asset->url,
    'content':body,
  }`;
  return await client.fetch(getProductQuery, {
    revalidate: new Date().getSeconds(),
  });
}

export async function getProductDetail(slug: string) {
  const getProductDetailQuery = groq`*[_type == "product"][slug.current == $slug][0]{
    'categories': categories[]->title,
    'slug':slug.current,
    publishedAt,
    title,
  ...
  return await client.fetch(getProductDetailQuery, {
    slug,
    revalidate: new Date().getSeconds(),
  });
}
